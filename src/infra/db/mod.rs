...
pub async fn setup_database(database_url: &str) -> Result<DatabasePool> {
        ...
        .after_connect(|conn, _meta| {
            Box::pin(async move {
                ...
                sqlx::query("SET statement_timeout = '60s'")
                    .execute(&mut *conn)
                    .await?; // Increased to 60s
                sqlx::query("SET lock_timeout = '10s'")
                    .execute(&mut *conn)
                    .await?; // Priority 4: Add lock timeout
                sqlx::query("SET idle_in_transaction_session_timeout = '60s'")
                    .execute(&mut *conn)
                    .await?; // Priority 4: Prevent long idle transactions
                             // Priority 4: Performance tuning settings
                sqlx::query("SET shared_preload_libraries = 'pg_stat_statements'")
                    .execute(&mut *conn)
                    .await
                    .ok(); // Enable query statistics
                sqlx::query("SET track_activity_query_size = 'on'")
                    .execute(&mut *conn)
                    .await
                    .ok(); // Track query activity
                Ok(())
            })
        })
    ...
    info!(
        "✅ Database connection established successfully in {:?}",
        connection_time
    );
...
pub async fn setup_timescale_database(influxdb_url: &str) -> Result<Option<DatabasePool>> {
    if influxdb_url.starts_with("http://") || influxdb_url.starts_with("https://") {
        info!(
            "InfluxDB connection skipped (HTTP URL detected): {}",
            influxdb_url
        );
