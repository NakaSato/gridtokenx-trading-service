...
impl SettlementService {
    ...
    pub async fn process_pending_settlements(&self) -> Result<usize> {
        ...
        if pending.len() > 5 {
            info!(
                "Truncating settlement batch from {} to 5 for Solana size limits",
                pending.len()
            );
            pending.truncate(5);
        }
    ...
    pub async fn process_settlements_batched(
        ...
        let tx_results = match self
            .provider
            .execute_batched_settlements(provider_inputs)
            .await
        {
            Ok(results) => results,
            Err(e) => {
            ...
            if let Some(erc) = &self.erc_service {
                    ...
                    .await
                {
                    if !certs.is_empty() {
                        ...
                        info!(
                            "Transferring ERC {} for settlement {}",
                            cert.certificate_id, settlement.id
                        );
                            ...
                            .await
                        {
                            ...
                            Err(e) => {
                                error!(
                                    "Failed to transfer ERC for settlement {}: {}",
                                    settlement.id, e
                                );
                            }
                        }
