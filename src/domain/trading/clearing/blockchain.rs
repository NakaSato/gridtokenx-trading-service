...
impl MarketClearingService {
    ...
    pub async fn fetch_and_decrypt_wallets_batch(
        ...
        info!(
            "🔐 Batch decrypting {} user wallets for matching engine",
            user_ids.len()
        );
        ...
        for user in users_data {
            ...
            let enc_key: Option<Vec<u8>> = user.get("encrypted_private_key");
            let iv: Option<Vec<u8>> = user.get("encryption_iv");
            let salt: Option<Vec<u8>> = user.get("wallet_salt");
            let user_id: Uuid = user.get("id");

            if let (Some(enc_key), Some(iv), Some(salt)) = (enc_key, iv, salt) {
                    ...
                    match WalletService::decrypt_private_key(
                        ...
                        Ok(pk_bytes) => {
                            if let Ok(kp) = Keypair::try_from(pk_bytes.as_slice()) {
                                ...
                                Err(anyhow::anyhow!("Invalid key for user {}", user_id))
                            }
                        }
                        Err(e) => Err(anyhow::anyhow!("Decryption failed for {}: {}", user_id, e)),
                    }
                }));
            }
        }
        ...
        for res in results {
            match res {
                Ok(Ok((id, kp))) => {
                    decrypted_wallets.insert(id, kp);
                }
    ...
    pub(super) async fn execute_on_chain_order_creation(
        ...
        if !self.config.tokenization.enable_real_blockchain {
            info!(
                "Blockchain processing is disabled. Skipping on-chain order creation for order {}",
                order_id
            );
            return Ok(());
        }
        ...
        let keypair = if let Some(key_bytes) = private_key_bytes {
            ...
            let enc_key: Option<Vec<u8>> = db_user.get("encrypted_private_key");
            let iv: Option<Vec<u8>> = db_user.get("encryption_iv");
            let salt: Option<Vec<u8>> = db_user.get("wallet_salt");
            ...
            if let (Some(enc_key), Some(iv), Some(salt)) = (enc_key, iv, salt) {
                ...
                let (enc_key_b64, salt_b64, iv_b64) = WalletService::encrypt_private_key(
                    master_secret,
                    master_secret,
        ...
        let (signature, order_pda, order_index) = if self.config.tokenization.enable_real_blockchain
        {
            ...
            let pda_opt = if pda_str.is_empty() {
                None
            } else {
                Some(pda_str)
            };
            ...
            return Err(anyhow::anyhow!(
                "Blockchain processing is disabled. Cannot create on-chain order."
            ));
        };
        ...
        if lock_amount > Decimal::ZERO {
            match self
                .execute_escrow_lock(user_id, order_id, lock_amount, asset_type, session_token)
                .await
            {
                Ok(sig) => {
                    info!(
                        "On-chain escrow lock executed for order {}: {}",
                        order_id, sig
                    );
                }
                Err(e) => {
                    error!(
                        "Failed to execute escrow lock for order {}: {}",
                        order_id, e
                    );
                }
            }
            ...
            info!(
                "Skipping on-chain escrow lock for order {} as amount is 0",
                order_id
            );
        }
    ...
    pub(super) async fn execute_escrow_lock(
        ...
        if !self.config.tokenization.enable_real_blockchain {
            return Err(anyhow::anyhow!(
                "Blockchain processing is disabled. Cannot execute escrow lock."
            ));
        }
        ...
        let user_ata = self
            .blockchain_service
            .ensure_token_account_exists(&api_authority, &keypair.pubkey(), &mint)
            .await?;
        ...
        let escrow_ata = self
            .blockchain_service
            .ensure_token_account_exists(&api_authority, &escrow_owner, &mint)
            .await?;
        ...
        info!(
            "Locking {} {} tokens ({} raw) from {} to API escrow {}",
            amount,
            asset_type,
            amount_u64,
            keypair.pubkey(),
    ...
    pub(super) async fn execute_escrow_release(
        ...
        if !self.config.tokenization.enable_real_blockchain {
            return Err(anyhow::anyhow!(
                "Blockchain processing is disabled. Cannot execute escrow release."
            ));
        }
        ...
        let db_user = sqlx::query("SELECT wallet_address FROM users WHERE id = $1")
            .bind(target_user_id)
            .fetch_optional(&self.db)
            .await?
        ...
        let escrow_ata = self
            .blockchain_service
            .calculate_ata_address(&escrow_owner, &mint)?;

        // 4. Ensure Receiver ATA exists
        let receiver_ata = self
            .blockchain_service
            .ensure_token_account_exists(&api_authority, &receiver_wallet, &mint)
            .await?;
        ...
        info!(
            "Releasing {} {} tokens from API escrow to receiver {}",
            amount, asset_type, receiver_wallet
        );
    ...
    pub(super) async fn execute_escrow_refund(
        ...
        if !self.config.tokenization.enable_real_blockchain {
            return Err(anyhow::anyhow!(
                "Blockchain processing is disabled. Cannot execute escrow refund."
            ));
        }
        ...
        let user_ata = self
            .blockchain_service
            .ensure_token_account_exists(&api_authority, &user_wallet, &mint)
            .await?;
        ...
        info!(
            "Refunding {} {} tokens from API escrow to user {}",
            amount, asset_type, user_wallet
        );
    ...
    pub(super) async fn execute_offchain_settlement(
        ...
        if !self.config.tokenization.enable_real_blockchain {
            return Err(anyhow::anyhow!(
                "Blockchain processing is disabled. Cannot execute off-chain settlement."
            ));
        }
        ...
        let buyer_currency_ata = self
            .blockchain_service
            .calculate_ata_address(&buyer_payload.user, &currency_mint)?;
        let seller_currency_ata = self
            .blockchain_service
            .calculate_ata_address(&seller_payload.user, &currency_mint)?;
        let seller_energy_ata = self
            .blockchain_service
            .calculate_ata_address(&seller_payload.user, &energy_mint)?;
        let buyer_energy_ata = self
            .blockchain_service
            .calculate_ata_address(&buyer_payload.user, &energy_mint)?;
        ...
        let fee_collector_ata = self
            .blockchain_service
            .ensure_token_account_exists(&api_authority, &collector_owner, &currency_mint)
            .await?;
        ...
        let wheeling_charge_u64 = (wheeling_charge * currency_multiplier)
            .to_u64()
            .unwrap_or(0);
