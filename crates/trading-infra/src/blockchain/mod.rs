...
impl BlockchainGateway for BlockchainService {
    ...
    async fn execute_settlement(
        ...
        if settlement.trade_id.is_none() {
            ...
            info!("Surplus detected: Executing generation mint for user {}", settlement.seller_id);
            
            let seller_wallet = self.get_user_primary_wallet(&settlement.seller_id).await
                .map_err(|e| trading_core::error::ApiError::Internal(format!("Failed to resolve seller wallet: {}", e)))?
                .ok_or_else(|| trading_core::error::ApiError::NotFound(format!("Primary wallet not found for user {}", settlement.seller_id)))?;

            let provider = BlockchainSettlementProvider::new(Arc::new(self.clone()));
            let signature = provider.execute_generation_mint(
                &seller_wallet,
            ...
            Err(trading_core::error::ApiError::Internal("Atomic swap settlement not yet fully wired to PDAs".to_string()))
        }
    }
    ...
    async fn issue_erc(
        ...
        info!("Issuing ERC for user {} (Meter: {}, Amount: {})", user_id, meter_id, energy_amount);
        
        let user_wallet = self.get_user_primary_wallet(&user_id).await
            .map_err(|e| trading_core::error::ApiError::Internal(format!("Failed to resolve user wallet: {}", e)))?
            .ok_or_else(|| trading_core::error::ApiError::NotFound(format!("Primary wallet not found for user {}", user_id)))?;

        let authority = self.get_authority_keypair().await
            .map_err(|e| trading_core::error::ApiError::Internal(format!("Failed to load authority: {}", e)))?;
        ...
        let amount_u64 = (energy_amount * rust_decimal::Decimal::from(1_000_000_000i64)).to_u64().unwrap_or(0);

        let signature = self.issue_erc(
            &cert_id,
            &user_wallet,
