    ...
    fn can_accommodate_flow(
        ...
        if let Some(target_zone) = to_zone {
            if let Some(config) = IslandRegistry::get_island_config(target_zone) {
                ...
                if (committed + new_flow_mw)
                    > Decimal::from_f64_retain(config.submarine_cable_capacity_mw)
        ...
        if from_zone == to_zone {
            ...
            if let Some(target) = to_zone {
                if IslandRegistry::get_island_config(target).is_some() {
                    return trading_core::fast_price::FastPrice::from(rust_decimal_macros::dec!(
                        0.05
                    ));
                }
            }
