...
#[must_use]
pub fn next_execution_at(from: DateTime<Utc>, interval_type: IntervalType, interval_value: i32) -> DateTime<Utc> {
...
mod tests {
    ...
    fn hourly_advances_hours() {
        ...
        assert_eq!(next_execution_at(base, IntervalType::Hourly, 1), at(2026, 6, 7, 11));
        assert_eq!(next_execution_at(base, IntervalType::Hourly, 6), at(2026, 6, 7, 16));
    }

    #[test]
    fn daily_advances_days() {
        let base = at(2026, 6, 7, 10);
        assert_eq!(next_execution_at(base, IntervalType::Daily, 1), at(2026, 6, 8, 10));
        assert_eq!(next_execution_at(base, IntervalType::Daily, 3), at(2026, 6, 10, 10));
    }

    #[test]
    fn weekly_advances_weeks() {
        let base = at(2026, 6, 7, 10);
        assert_eq!(next_execution_at(base, IntervalType::Weekly, 1), at(2026, 6, 14, 10));
        assert_eq!(next_execution_at(base, IntervalType::Weekly, 2), at(2026, 6, 21, 10));
    }

    #[test]
    fn monthly_advances_calendar_months() {
        let base = at(2026, 1, 15, 10);
        assert_eq!(next_execution_at(base, IntervalType::Monthly, 1), at(2026, 2, 15, 10));
        assert_eq!(next_execution_at(base, IntervalType::Monthly, 3), at(2026, 4, 15, 10));
    }
    ...
    fn monthly_clamps_end_of_month() {
        ...
        assert_eq!(next_execution_at(base, IntervalType::Monthly, 1), at(2026, 2, 28, 10));
    }

    #[test]
    fn zero_or_negative_interval_clamps_to_one() {
        let base = at(2026, 6, 7, 10);
        assert_eq!(next_execution_at(base, IntervalType::Daily, 0), at(2026, 6, 8, 10));
        assert_eq!(next_execution_at(base, IntervalType::Daily, -5), at(2026, 6, 8, 10));
    }
}
